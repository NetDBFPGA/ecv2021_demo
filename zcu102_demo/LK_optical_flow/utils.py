...
def show_frame(capture_frame, pred_result, fps, show_meta, topk=None, gt=None):
    ...
    for c in topk_class:
        class_str += f"{c} "
    result = f' fps:{"{:.1f}".format(fps)}, Predictions: {"".ljust(len(hit_class)*2)} {class_str}'

    frame = PIL.Image.fromarray(capture_frame)
    if show_meta:
        draw = ImageDraw.Draw(frame)
        draw.text((10,h-23), result, (255,255,255), font=font)
        draw.text((190,h-23), hit_class, (255,255,0), font=font)
    return frame

def showarray(capture_frame, fps=0, fmt='jpeg', show_meta=True, topk=None, gt=None):    
    f = io.BytesIO()
    
    # put your model here
    #pred_result = your_model(capture_frame)
    
    frame = show_frame(capture_frame, 0, fps, show_meta, topk, gt)
